...
static u8 *SSD1306_PrintChar(u8 *str)
{
        ...
        */
        ...
        if (*str != '\n')
        {
                ...
                for (i = 0; i < 8; i++)
                {
                        if (font_data[i] & BIT(7))
                                inv_data[0] |= BIT(i);
                        if (font_data[i] & BIT(6))
                                inv_data[1] |= BIT(i);
                        if (font_data[i] & BIT(5))
                                inv_data[2] |= BIT(i);
                        if (font_data[i] & BIT(4))
                                inv_data[3] |= BIT(i);
                        if (font_data[i] & BIT(3))
                                inv_data[4] |= BIT(i);
                        if (font_data[i] & BIT(2))
                                inv_data[5] |= BIT(i);
                        if (font_data[i] & BIT(1))
                                inv_data[6] |= BIT(i);
                        if (font_data[i] & BIT(0))
                                inv_data[7] |= BIT(i);
                }
...
static void SSD1306_bitmap(void)
{
        ...
        for (x = BITMAP_X_RES / 2; x < BITMAP_X_RES; x++)
        {
                ...
                {
                        if (read_bitmap(x, y))
                                inv_data[x - BITMAP_X_RES / 2] |= BIT(y);
                }
        }
        ...
        for (x = 0; x < BITMAP_X_RES / 2; x++)
        {
                ...
                {
                        if (read_bitmap(x, y))
                                inv_data[x] |= BIT(y - BITMAP_Y_RES / 2);
                }
        }
        ...
        for (x = BITMAP_X_RES / 2; x < BITMAP_X_RES; x++)
        {
                ...
                {
                        if (read_bitmap(x, y))
                                inv_data[x - BITMAP_X_RES / 2] |= BIT(y - BITMAP_Y_RES / 2);
                }
        }
                          ...
                          const struct i2c_device_id *id)
{
        ...
        SSD1306_String(" !\"#$%&'()*+,-./:;<=>?@[\\]^_`{|}~\n");
        SSD1306_String("ＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ\n");
        SSD1306_String("漢字もつかえちゃう\n");
